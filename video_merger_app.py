...
def make_snow_zoom_video(media_paths, output_path, fps=30,
    ...
    if not _install_pillow():
        return False, "Khong cai duoc Pillow. Chay: pip install Pillow"
    ...
    for idx, mpath in enumerate(media_paths):
        ...
        # For images: use PIL (handles EXIF rotation, all formats)
        # For videos: use ffprobe
        w, h = 0, 0
        if ext in IMAGE_EXTS_EDIT:
            try:
                from PIL import Image as _Img
                with _Img.open(mpath) as _im:
                    # Apply EXIF orientation so we get the DISPLAY size
                    from PIL import ImageOps as _IOs
                    _im = _IOs.exif_transpose(_im)
                    w, h = _im.size          # (width, height)
            except Exception:
                w, h = 0, 0
        if w == 0 or h == 0:
            # fallback: ffprobe
        ...
        if w > MAX_DIM or h > MAX_DIM:
            ...
            w = max(2, int(w*scale) if int(w*scale)%2==0 else int(w*scale)-1)
            h = max(2, int(h*scale) if int(h*scale)%2==0 else int(h*scale)-1)
        # Pre-convert image to normalized jpg so ffmpeg reads it cleanly
        if ext in IMAGE_EXTS_EDIT:
            try:
                from PIL import Image as _Img, ImageOps as _IOs
                norm_path = os.path.join(tmp, f"norm_{idx}.jpg")
                with _Img.open(mpath) as _im:
                    _im = _IOs.exif_transpose(_im)
                    _im = _im.convert("RGB")
                    _im = _im.resize((w, h), _Img.LANCZOS)
                    _im.save(norm_path, "JPEG", quality=95)
                mpath = norm_path
            except Exception:
                pass
        ...
        n_frames = int(fps * clip_dur)
        half     = n_frames // 2

        # ── Step 1: zoom in/out on source ─────────────────
        ...
        if ext in IMAGE_EXTS_EDIT:
            ...
            r = subprocess.run([
                "ffmpeg","-y","-i",mpath,"-filter_complex",fg_zoom,
                "-t",str(clip_dur),
                "-c:v","libx264","-preset","fast","-crf","23","-pix_fmt","yuv420p","-an",
                zoomed], capture_output=True)
        else:
            fg_zoom = (
        ...
        if r.returncode != 0:
            return False, f"Zoom that bai [{os.path.basename(mpath)}]:\n{r.stderr.decode()[-500:]}"

        # ── Step 2: generate snow overlay video ───────────
        st.caption(f"Tao tuyet cho clip {idx+1}...")
        snow_vid, snow_err = _make_snow_video(w, h, fps, n_frames, snow_count, idx*7+42, tmp)
        if not snow_vid:
            return False, f"Loi tao tuyet: {snow_err}"

        # ── Step 3: overlay snow on zoomed video ──────────
        r = subprocess.run([
            "ffmpeg","-y",
            "-i", zoomed,
            "-i", snow_vid,
