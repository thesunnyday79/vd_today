...
def download_file(url, dest):
    ...
    try:
        r = requests.get(url, stream=True, timeout=120, headers=hdrs, allow_redirects=True)
        r.raise_for_status()
        ct = r.headers.get("Content-Type", "")
        if "text/html" in ct:
            # Try to resolve MediaFire / redirect pages
            direct = _resolve_mediafire_url(url)
            if direct:
                r2 = requests.get(direct, stream=True, timeout=120, headers=hdrs, allow_redirects=True)
                r2.raise_for_status()
                if "text/html" in r2.headers.get("Content-Type", ""):
                    st.warning(f"Khong resolve duoc link thuc: {url[:80]}")
                    return False
                size = 0
                with open(dest, "wb") as f:
                    for chunk in r2.iter_content(512*1024):
                        f.write(chunk); size += len(chunk)
                return size > 200
            else:
                st.warning(f"URL tra ve HTML, khong lay duoc file: {url[:80]}")
                return False
        size = 0
        with open(dest, "wb") as f:
        ...
        st.warning(f"HTTP {e.response.status_code}: {url[:80]}")
        return False
    except Exception as e:
        st.warning(f"Loi tai: {e} | {url[:80]}")
        return False
...
def make_fire_particles_video(media_paths, output_path, fps=30,
                               ...
                               audio_path=None):
    """
    Each media file gets animated fire particles (embers/sparks rising from bottom).
    Particles: small circles in yellow/orange/red, rising upward with wobble,
    fading out as they rise — exactly like the reference image.
    """
    import random, math, shutil
    from PIL import Image as PILImg, ImageDraw as PILDraw, ImageOps as PILOps, ImageFilter as PILFilter
    ...
    # Fire particle palette: yellow core → orange → red rim
    FIRE_COLORS = [
        (255, 240, 80),   # bright yellow
        (255, 210, 40),   # yellow-orange
        (255, 170, 20),   # orange
        (255, 130, 10),   # deep orange
        (255, 90,  10),   # orange-red
        (255, 60,  10),   # red-orange
    ]

    rng = random.Random(99)

    def _gen_particles(w, h, count):
        """Generate particle definitions."""
        particles = []
        for _ in range(count):
            particles.append({
                "x":      rng.uniform(0.05, 0.95),   # start x (fraction of w)
                "y":      rng.uniform(0.7, 1.05),    # start y — from bottom area
                "vy":     rng.uniform(0.06, 0.22),   # rise speed (fraction/sec)
                "vx":     rng.uniform(-0.012, 0.012),# horizontal drift
                "wobble": rng.uniform(0.005, 0.02),  # horizontal oscillation amp
                "wfreq":  rng.uniform(1.5, 4.0),     # wobble frequency Hz
                "r":      rng.uniform(2.5, 6.0),     # radius px
                "color":  rng.choice(FIRE_COLORS),
                "phase":  rng.uniform(0, 1.0),       # time offset (stagger birth)
                "life":   rng.uniform(0.5, 1.2),     # lifetime in seconds
            })
        return particles
    ...
    for idx, mpath in enumerate(media_paths):
        ...
        n_frames = max(1, int(fps * clip_dur))
        half = n_frames // 2
        particles = _gen_particles(w, h, particle_count)
        ...
        if ext in IMAGE_EXTS_EDIT:
            try:
                with PILImg.open(mpath) as im:
                    im = PILOps.exif_transpose(im).convert("RGB").resize((w,h), PILImg.LANCZOS)
                    src_frame = im.copy()
            except Exception as e:
                return False, f"Khong mo duoc anh [{os.path.basename(mpath)}]: {e}"
            ...
            for fi in range(n_frames):
                ...
                x0 = (w-cw)//2; y0 = (h-ch)//2
                frame = src_frame.crop((x0,y0,x0+cw,y0+ch)).resize((w,h), PILImg.LANCZOS).copy()

                # Draw fire particles on frame
                draw = PILDraw.Draw(frame, "RGBA")
                for p in particles:
                    # Each particle cycles with its phase offset
                    cycle_t = (t + p["phase"] * p["life"]) % p["life"]
                    age = cycle_t / p["life"]          # 0=born, 1=dead

                    # Position: rises upward, wobbles sideways
                    px = (p["x"] + p["vx"]*cycle_t +
                          p["wobble"] * math.sin(2*math.pi*p["wfreq"]*cycle_t)) * w
                    py = (p["y"] - p["vy"]*cycle_t) * h

                    # Skip if out of frame
                    if py < -20 or px < -20 or px > w+20:
                        continue

                    # Fade: bright when young, fades as rises
                    fade = max(0.0, 1.0 - age)         # linear fade
                    alpha = int(220 * fade * fade)      # quadratic fade
                    if alpha < 10:
                        continue
                    ...
                    col = p["color"] + (alpha,)         # RGBA

                    # Draw glow: larger semi-transparent outer ring
                    glow_r = r_int + 3
                    glow_alpha = alpha // 4
                    draw.ellipse(
                    ...
                    # Core bright dot
                    draw.ellipse(
                        [px-r_int, py-r_int, px+r_int, py+r_int],
                        fill=col
                    )
                    # Bright white center
                    if r_int >= 2:
                        draw.ellipse(
                            [px-1, py-1, px+1, py+1],
                            fill=(255, 255, 240, min(255, alpha+40))
                        )
            ...
            for fi in range(n_frames):
                ...
                for p in particles:
                    ...
                    px = (p["x"] + p["vx"]*cycle_t +
                          p["wobble"]*math.sin(2*math.pi*p["wfreq"]*cycle_t)) * w
                    py = (p["y"] - p["vy"]*cycle_t) * h
                    if py < -20 or px < -20 or px > w+20: continue
                    fade = max(0.0, 1.0-age)
                    alpha = int(220*fade*fade)
                    ...
                    draw.ellipse([px-glow_r,py-glow_r,px+glow_r,py+glow_r],
                                 fill=p["color"]+(alpha//4,))
                    draw.ellipse([px-r_int,py-r_int,px+r_int,py+r_int],
                                 fill=p["color"]+(alpha,))
                    if r_int >= 2:
    ...
    # ── Concat ────────────────────────────────────────────
    st.info("Ghep clip...")
    ctxt = os.path.join(tmp, "fire_concat.txt")
    with open(ctxt,"w") as f:
        f.write("\n".join(f"file '{c}'" for c in clips))
    cout = os.path.join(tmp, "fire_concat.mp4")
    r = subprocess.run(
        ["ffmpeg","-y","-f","concat","-safe","0","-i",ctxt,"-c","copy",cout],
        capture_output=True)
    if r.returncode != 0:
    ...
    if audio_path and os.path.exists(audio_path):
        ...
        aac = os.path.join(tmp,"fire_audio.aac")
        subprocess.run(["ffmpeg","-y","-i",audio_path,"-vn","-c:a","aac",
                        "-b:a","192k","-ar","44100","-ac","2",aac], capture_output=True)
        adj = os.path.join(tmp,"fire_adj.mp4")
        if aud_dur and vid_dur and aud_dur > vid_dur:
            subprocess.run(["ffmpeg","-y","-stream_loop","-1","-i",cout,
                            "-t",str(aud_dur),"-c:v","libx264","-preset","fast",
                            "-crf","22","-an",adj], capture_output=True)
        else:
            subprocess.run(["ffmpeg","-y","-i",cout,
                            "-t",str(aud_dur or vid_dur),"-c:v","libx264",
                            "-preset","fast","-crf","22","-an",adj], capture_output=True)
        r = subprocess.run([
            "ffmpeg","-y","-i",adj,"-i",aac,
            "-c:v","copy","-c:a","aac","-b:a","192k",
            "-map","0:v:0","-map","1:a:0","-shortest",output_path
    ...
    if r.returncode != 0:
        return False, f"Loi ghep audio:\n{r.stderr.decode()[-400:]}"
    return True, "Fire particles video hoan tat!"
...
Hỗ trợ: ảnh <code>jpg png webp</code> và video <code>mp4 mov</code>
    ...
    with fc2: fire_particles = st.slider("Số đốm lửa", 20, 150, 60, 10, key="fire_count")
    with fc3: fire_zoom = st.slider("Mức zoom (%)", 0, 20, 8, 1, key="fire_zoom")
    fire_out_name = st.text_input("Tên file output", value="fire_effect.mp4", key="fire_out_name")
    ...
    if not local_fire_media:
        ...
        if st.button("🔥 Tạo Fire Video", use_container_width=True, type="primary", key="btn_fire"):
            ...
            if ok_f:
                st.markdown('<div class="success-box">🔥 Fire video hoàn tất!</div>', unsafe_allow_html=True)
