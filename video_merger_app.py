...
def _download_dropbox_with_token(url, dest, token):
    """Download Dropbox shared link using API token — works even for non-public files."""
    import json as _json
    try:
        # Use sharing/get_shared_link_file API
        r = requests.post(
            "https://content.dropboxapi.com/2/sharing/get_shared_link_file",
            headers={
                "Authorization": f"Bearer {token}",
                "Dropbox-API-Arg": _json.dumps({"url": url}),
        ...
        if r.status_code != 200:
            return False, f"Dropbox API {r.status_code}: {r.text[:200]}"
        size = 0
        with open(dest, "wb") as f:
            for chunk in r.iter_content(512*1024):
                f.write(chunk); size += len(chunk)
        if size < 200:
            return False, f"File rong ({size}B)"
        return True, None
    except Exception as e:
        return False, str(e)

def _download_gdrive(url, dest):
    """Download Google Drive file (public share link)."""
    import re
    m = re.search(r"drive\.google\.com/file/d/([^/\?]+)", url)
    if not m:
        m = re.search(r"id=([^&]+)", url)
    if not m:
        return False, "Khong lay duoc file ID tu Google Drive URL"
    file_id = m.group(1)
    direct = f"https://drive.google.com/uc?export=download&confirm=t&id={file_id}"
    hdrs = {"User-Agent": "Mozilla/5.0"}
    try:
        r = requests.get(direct, stream=True, timeout=300, headers=hdrs, allow_redirects=True)
        r.raise_for_status()
        size = 0
        with open(dest, "wb") as f:
            for chunk in r.iter_content(512*1024):
...
def download_file(url, dest, dbx_token=None):
    """
    Smart downloader — handles Dropbox (via API token), Google Drive, Cloudinary, direct URLs.
    """
    url = url.strip()
    BROWSER_UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
    hdrs = {"User-Agent": BROWSER_UA}

    # ── Dropbox: always use API with token ───────────────
    if _is_dropbox_url(url):
        # Get fresh token from session state if not passed
        token = dbx_token
        if not token:
            token = st.session_state.get("dbx_token")
        if not token:
            st.warning(f"⚠️ Can dang nhap Dropbox de tai file: {url[:80]}")
            return False
        # Auto-refresh token
        ...
        ok, err = _download_dropbox_with_token(url, dest, token)
        if not ok:
            st.warning(f"⚠️ Dropbox download that bai: {err} | {url[:80]}")
        return ok

    # ── Google Drive ──────────────────────────────────────
    if "drive.google.com" in url:
        ok, err = _download_gdrive(url, dest)
        if not ok:
            st.warning(f"⚠️ Google Drive: {err}")
        return ok
    ...
    try:
        ...
        with open(dest, "wb") as f:
            for chunk in r.iter_content(512*1024):
                f.write(chunk); size += len(chunk)
        if size < 200:
            st.warning(f"⚠️ File rong ({size}B): {url[:80]}")
            return False
        return True
