...
def download_file(url, dest, dbx_token=None):
    ...
    - Dropbox links: NOT supported (server proxy blocks dropboxapi.com)
      → User must upload files directly instead.
    - Cloudinary, direct CDN links: supported
    - MediaFire: attempt HTML parse
    """
    url = url.strip()
    BROWSER_UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
    hdrs = {"User-Agent": BROWSER_UA}

    # ── Dropbox: BLOCKED by server proxy ─────────────────
    if "dropbox.com" in url:
        st.warning(
            f"⚠️ **Dropbox link không tải được từ server** — môi trường Streamlit Cloud "
            f"chặn kết nối tới Dropbox API.\n\n"
            f"✅ **Giải pháp:** Dùng tab **📁 Upload trực tiếp** để tải file từ máy tính lên."
        )
        return False

    # ── MediaFire: parse HTML ─────────────────────────────
    ...
    try:
        ...
        if "text/html" in ct:
            st.warning(
                f"⚠️ URL trả về trang HTML, không phải file.\n"
                f"Chỉ dùng được link CDN trực tiếp (Cloudinary, v.v.) "
                f"hoặc **Upload trực tiếp** từ máy tính."
            )
            return False
        size = 0
...
tab1,tab2,tab3,tab4,tab5,tab6=st.tabs(["🔗 Nhập URL","📋 Danh sách","🚀 Xuất video","✨ Edit video","🔥 Hiệu ứng lửa","📦 Bulk Video"])
