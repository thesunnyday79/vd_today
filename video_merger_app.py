...
def merge_videos_and_audio(
    ...
    """
    Merge multiple videos, then overlay/replace with audio tracks.
    ...
    # Re-encode each video to ensure compatibility
    reencoded = []
    progress = st.progress(0, text="Re-encoding videos for compatibility…")
    ...
    if audio_paths:
        ...
        # No external audio → just output merged video
...
if not check_ffmpeg():
    st.markdown("""
    <div class="error-box">
    ⚠️ <b>FFmpeg chưa được cài đặt.</b><br>
    Chạy lệnh: <code>sudo apt-get install ffmpeg</code> hoặc <code>brew install ffmpeg</code>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
